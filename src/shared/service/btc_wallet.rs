...
pub fn generate_bitcoin_wallet(
    ...
    let xprv = xkey.into_xprv(Network::Bitcoin).ok_or_else(|| {
        AppError::InternalServerError("Failed to create extended private key".to_string())
    })?;

    // 3) Use BIP84 (P2WPKH bech32). Coin type: 0' (mainnet) / 1' (testnet)
    let coin_type: u32 = if matches!(Network::Bitcoin, Network::Bitcoin) {
        0
    } else {
        1
    };
    ...
    let address = Address::p2wpkh(&public_key, Network::Bitcoin)
        .map_err(|_| {
            AppError::InternalServerError("Failed to generate P2WPKH address".to_string())
        })?
        .to_string();
    println!(
        "mnemonic: {} \n, address: {} \n, Private_key: {} \n",
