...
use bitcoin::{
    ...
    Address, EcdsaSighashType, Network, OutPoint, PrivateKey, PublicKey, Script, Transaction, TxIn,
    TxOut,
};
use rand;
...
impl BtcWithdrawalService {
    ...
    pub async fn execute_withdrawal(
        ...
        let private_key_wif = encryption.decrypt(&wallet.private_key).map_err(|e| {
        ...
        let private_key = PrivateKey::from_wif(&private_key_wif).map_err(|e| {
            AppError::InternalServerError(format!("Invalid private key format: {}", e))
        })?;
        ...
        log::info!(
            "💰 Total balance: {} satoshis across {} UTXOs",
            total_balance,
            utxos.len()
        );
        ...
        let inputs_used: Vec<String> = selected_utxos
            .iter()
    ...
    async fn get_merchant_wallet(
        ...
        let wallet = wallet::Entity::find()
            ...
            .ok_or_else(|| {
                AppError::ValidationError("No Bitcoin wallet found for merchant".to_string())
            })?;
    ...
    async fn fetch_utxos(address: &str, environment: &str) -> Result<Vec<Utxo>, AppError> {
        ...
        let response =
            client.get(&url).send().await.map_err(|e| {
                AppError::InternalServerError(format!("Failed to fetch UTXOs: {}", e))
            })?;

        if !response.status().is_success() {
            return Err(AppError::InternalServerError(
                "Failed to fetch UTXOs from API".to_string(),
            ));
        }

        let utxo_data: Vec<serde_json::Value> = response.json().await.map_err(|e| {
            AppError::InternalServerError(format!("Failed to parse UTXO response: {}", e))
        })?;

        let utxos: Vec<Utxo> = utxo_data
            .iter()
            .map(|u| Utxo {
    ...
    fn select_utxos(utxos: &[Utxo], target_amount: u64) -> Result<(Vec<Utxo>, u64), AppError> {
        ...
        Err(AppError::ValidationError(
            "Insufficient UTXOs to cover amount and fees".to_string(),
        ))
    }
    ...
    async fn broadcast_transaction(raw_tx: &str, environment: &str) -> Result<String, AppError> {
        ...
        let response = client
            ...
            .await
            .map_err(|e| {
                AppError::InternalServerError(format!("Failed to broadcast transaction: {}", e))
            })?;
        ...
        let tx_hash = response.text().await.map_err(|e| {
            AppError::InternalServerError(format!("Failed to get transaction hash: {}", e))
        })?;
